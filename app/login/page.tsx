...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast("Gagal login: Periksa kembali email dan password Anda.", "error");
      setLoading(false); 
    } 
  };
    ...
    <main className="min-h-screen relative flex items-center justify-center p-6 bg-gradient-to-br from-blue-50 via-slate-50 to-indigo-100">
      ...
      <div className="w-full max-w-md bg-white/80 backdrop-blur-lg rounded-[2rem] shadow-2xl border border-white p-8 sm:p-10">
        <div className="text-center mb-10">
          ...
          <h2 className="text-lg font-bold text-slate-700">Selamat Datang Kembali!</h2>
        ...
        <p className="mt-8 text-center text-sm font-medium text-slate-500">
          Belum punya akun? <Link href="/register" className="text-blue-600 font-bold hover:text-blue-700 hover:underline transition-colors">Daftar di sini</Link>
        </p>
      </div>
    </main>
  );
